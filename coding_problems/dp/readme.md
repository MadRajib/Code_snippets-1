...
Apporach:
- when puting a book in new shelf, think about the new height and new shelf width
...
public:
    ...
    int minHeightShelves(vector<vector<int>>& books, int shelfWidth) {
        M_SW = shelfWidth;
        return rr(0, shelfWidth, 0, books);
    }

    int rr(int i, int SW, int maxH, vector<vector<int>>& books) {
    
        if (i >= books.size())
            return maxH;
        
        // skip current shelf, then we must place the book in the next shelf
        // therefore next shelf widht becomes shelfwidth - book.w
        // for new shelf new max height is book height itself
        int res = maxH + rr(i + 1, M_SW - books[i][0], books[i][1], books);
